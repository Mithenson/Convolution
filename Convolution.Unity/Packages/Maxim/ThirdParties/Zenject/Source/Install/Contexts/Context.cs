...
//#endif
        ...
        protected async UniTask InstallInstallers()
        {
            await InstallInstallers(_normalInstallers, _normalInstallerTypes, _scriptableObjectInstallers, _monoInstallers, _installerPrefabs);
        }
...
#endif
                {
                    await installer.InstallBindings();
                }
            }
        ...
        protected async UniTask InstallSceneBindings(List<MonoBehaviour> injectableMonoBehaviours)
        {
            ...
            foreach (var binding in Resources.FindObjectsOfTypeAll<ZenjectBinding>())
            {
                ...
                {
                    await InstallZenjectBinding(binding);
                }
            }
        }

        UniTask InstallZenjectBinding(ZenjectBinding binding)
        {
            if (!binding.enabled)
            {
