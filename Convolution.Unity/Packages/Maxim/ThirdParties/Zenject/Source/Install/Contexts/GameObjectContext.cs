...
namespace Zenject
{
    public class GameObjectContext : RunnableContext
    {
        ...
        private async UniTask Awake() => await Initialize();

        protected override async UniTask RunInternal()
        {
            await Install(_parentContainer);
            ResolveAndStart();
        }

        public async UniTask Install(DiContainer parentContainer) 
        {
            ...
            try
            {
                await InstallBindings(injectableMonoBehaviours);
            }
            finally
            {
        ...
        async UniTask InstallBindings(List<MonoBehaviour> injectableMonoBehaviours)
        {
            ...
            await InstallSceneBindings(injectableMonoBehaviours);
            await InstallInstallers();
        }
    }
}
